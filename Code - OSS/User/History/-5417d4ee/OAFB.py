...
#files = glob.glob("")
files = glob.glob("/data2/MPHY_Run43/902/Run43_0718_Sapphire902-ABCD_slot5_DG50_QET50pct_PTon_Pulse/*.txt")

saveF = ROOT.TFile("Run43_0718_Sapphire902-ABCD_slot5_DG50_QET50pct_PTon_Pulse.root","RECREATE")
...
for filen in files:
  ...
  if True:
      ...
      ra1 = [0.5 * scale, 1 * scale]
      ra2 = [1 * scale, 1.5 * scale]
      ra3 = [1.5 * scale, 2 * scale]
      ra4 = [2 * scale, 2.5 * scale]
      ra5 = [2.5 * scale, 3 * scale]
      ...
      if stdA < 0.1 * scale and stdA > 0 * scale and stdA2 < 0.1 * scale and stdA2 > 0 * scale and minA > -0.05 * scale and maxtailA < 0.05 * scale:
