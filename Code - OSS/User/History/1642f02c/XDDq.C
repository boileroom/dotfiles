...
void readSampleData(){
TFile * f = new TFile("Run43_0721_slot5_Sapphire903-ABCD_DG50_QET50pct_NoProbeTune_TrigC_PTon_Pulse_fitPhonon.root");
TTree * t=(TTree*)f->Get("data");


Double_t amp1,amp2,amp3,amp4,chi21,chi22,chi23,chi24,eve.xntT;			

TH1F* sapphire = new TH1F("sapphire","",400,0e-6, 50e-06);
...
for(Int_t i=0; i<t->GetEntries(); i++){
  ...
  t->SetBranchAddress("partXOF",&partX);
  t->SetBranchAddress("partYOF",&partY);
  t->SetBranchAddress("delayXOF",&delX);
  t->SetBranchAddress("delayYOF",&delY);

  sapphire->Fill((amp1+amp2+amp3+amp4));
  sapphire_calibrated->Fill(60*(amp1+amp2+amp3+amp4)/3.39130e-06);
